...
drop database db_teste_trigger;

create TABLE bkupdate (
...
DROP TRIGGER tg_Insert_Funcionarios 
...
BEGIN 
    INSERT INTO funcionariosBkp (
        data_mod, id, nome, telefone, email, data_nascimento, rua, bairro, cidade, estado, estado_sigla, cargo, salario
    ) 
    VALUES (
        NOW(), OLD.id, OLD.nome, OLD.telefone, OLD.email, OLD.data_nascimento, OLD.rua, OLD.bairro, OLD.cidade, OLD.estado, OLD.estado_sigla, OLD.cargo, OLD.salario
    );
END
$
DELIMITER ;
...
DELIMITER $
CREATE TRIGGER tg_Insert2_bkupdate
BEFORE UPDATE
ON bkupdate
FOR EACH ROW
BEGIN 
...
(42,'1989-08-22','Luna Teixeira','+55 (041) 0198-6798','aliceviana@hotmail.com','1990-01-30','Condomínio Emanuella Souza, 91','Glória','Amapá','Santa Catarina','AC','Agente de combate à endemias',2585.97),
...
DELETE FROM BKUPDATE WHERE id = 1;
